...
assert(D->size>2); // Det giver ikke rigtig mening at se på en matrice som er mindre end n=3 i denne problemstilling
assert(m>=0);

	// Vi starter med at opdatere D, så hvis den p'te indgang i u ikke er 0, opdateres D, og u sættes lig 0.
double dstart=gsl_vector_get(D,p);
double ustart=gsl_vector_get(u,p);
gsl_vector_set(D,p,dstart+2*ustart);
gsl_vector_set(u,p,0);

	// Vi skal have en ordnet udgave af D for at kunne bestemme intervallerne vi vil lede efter eigenvalues i.
...
while(pnew<=Dcopy->size){
	...
	// Erklærer de relevante doubles og ints og laver funktionen Se som ud fra D og u danner den relevante Secular Equation(4.30 fra bogen)
double a,xi,di,dI,gi;
int I;
gsl_blas_ddot(u,u,&a); //u skal bruges som øvre og nedre grænse for hvor vores egenværdier er

	// Laver det karakteristiske polynomium
...
gsl_vector* g=gsl_vector_alloc(1);
	...
	//Den mindste egenværdi bestemmes.
gsl_vector_set(x,0,NAN); 	//Hvis der ikke findes en egenværdi
...
while(I<=2*m+1){
	...
	// Intervallet efter d_p springes over, derfor er der lavet to forløkker
...
while(I<=2*m+1){
	...
	//Der frigives hukommelse, og nu burde alle egenværdier være fundet og gemt i x, eller nans hvis den ikke fandt nogen i et givet interval.
	gsl_vector_free(g);gsl_vector_free(Dcopy);
}
